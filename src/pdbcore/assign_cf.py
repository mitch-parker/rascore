...
from util.data import merge_tables
from util.path import save_table
from util.coord import load_cif_dict, search_cif_dict
...
def add_cf(df, min_simi=0.05):
    ...
    for index in tqdm(
        list(df.index.values), desc="Assigning crystal forms", position=0, leave=True
    ):
...
def assign_cf(coord_paths, cf_table_path=None, min_simi=0.05, data=None, num_cpu=1):
